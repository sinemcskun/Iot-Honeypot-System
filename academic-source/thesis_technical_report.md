...
┌──────────────────────────────────────────────────────────────┐
│  Dataset Generation                                          │
│    parse_cowrie_tty.py → fine_tune_dataset.jsonl (275)       │
│    generate_synthetic_dataset.py → synthetic (123 cmd +      │
│      7,605 login-failed + 3 file-download)                   │
│    Rebalanced & merged → combined_finetune_dataset.jsonl     │
│      (365 entries, entropy 3.89 bits, 68.6% efficiency)      │
│                         ↓                                    │
│  Model Training & Evaluation                                 │
│    train_phi3_cowrie.py → phi3-cowrie-lora-adapter           │
│    evaluate_phi3_cowrie.py → BLEU/ROUGE/Consistency/         │
│                              Hallucination Rate               │
└──────────────────────────────────────────────────────────────┘
```
...
- **`parse_cowrie_tty.py`**: Extracts raw attacker keystrokes and system responses from 341 binary Cowrie TTY log files, stripping ANSI escape sequences to produce 275 real command-response pairs (`fine_tune_dataset.jsonl`).
- **`generate_synthetic_dataset.py`**: Extends the dataset by processing all unique Cowrie commands from `Processed_Data.csv` (123 unique commands), plus `ssh_login_failed` events (7,605 unique user/password pairs modeled as `su <user>` failures) and `ssh_file_downloaded` events (3 unique URLs modeled as `wget`/`curl` outputs) into `synthetic_finetune_dataset.jsonl`.
- **Dataset Rebalancing**: The raw combined dataset (8,006 entries) suffered from severe class imbalance — 85.8% were `su` login-failure entries with identical output (`Password: \nsu: Authentication failure`), yielding a Shannon entropy of only 1.14 bits (19.8% efficiency). To prevent overfitting, `su` entries were capped at 50 (diverse usernames preserved), empty outputs removed, and the dataset shuffled. The final `combined_finetune_dataset.jsonl` contains **365 deduplicated entries** with entropy of **3.89 bits (68.6% efficiency)** across 51 distinct base commands.
- **`train_phi3_cowrie.py`**: Fine-tunes `microsoft/Phi-3-mini-4k-instruct` using QLoRA (4-bit quantization, LoRA rank=16, alpha=32) on a Google Colab T4 GPU. Saves the adapter as `phi3-cowrie-lora-adapter`.
- **`evaluate_phi3_cowrie.py`**: Evaluates the adapter against ground-truth samples using four metrics: **BLEU**, **ROUGE** (1/2/L), **Consistency Rate**, and **Hallucination Rate**.
...
The fine-tuned model was evaluated using four quantitative metrics, each measuring a different aspect of honeypot deception quality:

1. **BLEU Score** (Bilingual Evaluation Understudy): Measures corpus-level n-gram overlap between generated outputs and ground-truth Cowrie responses. Calculated using NLTK `corpus_bleu` with smoothing (Method 1). Higher is better.
2. **ROUGE Scores** (Recall-Oriented Understudy for Gisting Evaluation): ROUGE-1 (unigram), ROUGE-2 (bigram), and ROUGE-L (longest common subsequence) F1 scores averaged across all test samples. Computed using Google's `rouge-score` library with stemming enabled. Higher is better.
3. **Consistency Rate**: Each test prompt is sent to the model 3 times. If all 3 outputs are identical, the sample is classified as "consistent". Consistency Rate = (consistent samples / total samples) × 100%. Measures output stability and determinism at low temperature (T=0.1). Higher is better.
4. **Hallucination Rate**: A sample is classified as "hallucinated" if its per-sample ROUGE-L F1 score falls below a threshold (0.30), indicating the generated output has minimal overlap with the expected ground truth. Hallucination Rate = (hallucinated samples / total samples) × 100%. Lower is better.

#### 7.5.1 Evaluation Results

The evaluation was performed on the fine-tuned adapter (`phi3-cowrie-lora-adapter`) loaded with 4-bit quantization on a local GPU. Ground-truth samples were drawn from `combined_finetune_dataset.jsonl`.

| Metric | Value | Interpretation |
|---|---|---|
| **Corpus BLEU** | 0.1631 | Low n-gram overlap; model produces semantically related but lexically different outputs |
| **ROUGE-1 (F1)** | 0.2426 | ~24% unigram overlap with ground truth |
| **ROUGE-2 (F1)** | 0.1989 | ~20% bigram overlap |
| **ROUGE-L (F1)** | 0.2325 | ~23% longest common subsequence match |
| **Consistency Rate** | **90.0%** | 9/10 test commands produced identical output across 3 runs |
| **Hallucination Rate** | **76.7%** | 23/30 samples had ROUGE-L < 0.30 |

#### 7.5.2 Result Interpretation

**Consistency (90.0% — Strong):** The model demonstrates high output stability. At temperature 0.1, 9 out of 10 randomly selected prompts produced identical outputs across 3 inference runs. This is critical for honeypot deployment — an inconsistent model would produce different responses to the same command within a session, potentially alerting attackers.

**BLEU/ROUGE (Low — Expected):** The low BLEU (0.16) and ROUGE-L (0.23) scores require contextual interpretation. These metrics measure exact textual overlap with ground-truth Cowrie responses. Several factors contribute to low scores:

- **Dataset diversity**: The 365-entry training set covers 51 different base commands. With limited examples per command, the model generalizes rather than memorizes exact responses.
- **Legitimate variation**: For commands like `uname -a`, minor formatting differences (whitespace, field ordering) significantly penalize BLEU/ROUGE despite semantically correct output.
- **Command complexity**: Many test samples involve long chained commands (`apt update && apt install...`, `chmod +x ... ; nohup ...`) where partial response correctness is penalized harshly by corpus-level BLEU.
- **Baseline context**: For a 3.8B parameter model fine-tuned on 365 samples with QLoRA, these scores reflect expected behavior. Comparable studies on domain-specific fine-tuning with small datasets report BLEU scores of 0.10–0.25.

**Hallucination Rate (76.7% — High, but Nuanced):** The 76.7% hallucination rate indicates that most generated outputs differ substantially from ground truth. However, this metric uses a ROUGE-L threshold (< 0.30) which conflates two phenomena:
1. **True hallucination**: The model fabricates outputs (e.g., invents file paths, generates non-Linux responses).
2. **Paraphrasing**: The model produces a valid Linux terminal response that is textually different from the specific ground-truth template.

Manual inspection of the 30 evaluation samples reveals that many "hallucinated" outputs are structurally plausible Linux responses (e.g., producing a different error message format for `su` failures, or including extra output lines). This suggests the effective true hallucination rate is lower than 76.7%, though exact measurement would require expert manual review.

### 7.6 Implementation Steps for LLM Component

The development of the LLM component followed a structured pipeline of dataset preparation, quality assurance, model fine-tuning, and evaluation. All steps have been completed.

1. **TTY Log Parsing** (`parse_cowrie_tty.py`): 341 binary Cowrie TTY log files were parsed to extract 275 real command-response pairs. The parser handles ANSI escape sequences, backspace reconstruction, and state-based input/output separation. Output: `fine_tune_dataset.jsonl`.

2. **Synthetic Dataset Generation** (`generate_synthetic_dataset.py`): The script processes `Processed_Data.csv` to extract:
   - **123 unique commands** (from 110,000 Cowrie command rows) matched against Cowrie's local response files (`honeyfs/`, `txtcmds/`) and hardcoded dynamic handlers.
   - **7,605 unique login-failed entries** (`ssh_login_failed` → `su <username>` instruction with `Password: \nsu: Authentication failure` output).
   - **3 unique file-download entries** (`ssh_file_downloaded` → `wget`/`curl` instruction with simulated download output).
   Output: `synthetic_finetune_dataset.jsonl`.

3. **Dataset Quality Assurance & Rebalancing**: Information entropy analysis revealed critical class imbalance in the raw combined dataset (8,006 entries):
   - **Problem**: 85.8% of entries were `su <user>` with identical output. Shannon entropy: 1.14 bits (19.8% efficiency). Only 152 unique outputs for 2,282 entries.
   - **Fix**: `su` entries capped to 50 (diverse usernames preserved), empty outputs removed, entries shuffled.
   - **Result**: 365 entries, 151 unique outputs, 51 base commands, entropy 3.89 bits (68.6% efficiency).

4. **Model Selection & Fine-Tuning** (`train_phi3_cowrie.py`): The `microsoft/Phi-3-mini-4k-instruct` (3.8B parameters) was fine-tuned using QLoRA on a Google Colab T4 GPU (16GB VRAM):
   - 4-bit NF4 quantization with double quantization
   - LoRA adapter: rank=16, alpha=32, dropout=0.05
   - Training: 5 epochs via `SFTTrainer`
   - Output: `phi3-cowrie-lora-adapter/`

5. **Model Evaluation** (`evaluate_phi3_cowrie.py`): The adapter was loaded with 4-bit quantization and evaluated on 30 ground-truth samples (BLEU, ROUGE, Hallucination) and 10 samples × 3 runs (Consistency). Results: BLEU=0.1631, ROUGE-L=0.2325, Consistency=90.0%, Hallucination=76.7%. See Section 7.5.1 for detailed interpretation.
...
### Phase 5 — LLM-Enhanced Deception (Completed — Evaluation Phase)
...
| Component | Status |
|---|---|
| TTY Log Parsing (275 real command-response pairs) | ✅ Completed |
| Synthetic Dataset Generation (123 cmd + 7,605 login + 3 download) | ✅ Completed |
| Dataset Quality Assurance & Rebalancing (entropy 1.14→3.89 bits) | ✅ Completed |
| Combined Dataset (365 entries, 51 base commands, 151 unique outputs) | ✅ Completed |
| LLM Fine-Tuning (Phi-3-mini, QLoRA, Google Colab T4, 5 epochs) | ✅ Completed |
| Evaluation: BLEU + ROUGE (BLEU=0.16, ROUGE-L=0.23) | ✅ Completed |
| Evaluation: Consistency Rate (90.0%) | ✅ Completed |
| Evaluation: Hallucination Rate (76.7%) | ✅ Completed |
| LLM integration architecture | ⬜ Designed (see Section 7.2) |
...
**Q9: Why is the LLM's BLEU score so low (0.16)?**

The low BLEU score must be interpreted within context. BLEU measures exact n-gram overlap, which penalizes any lexical deviation — even when the generated output is a valid Linux terminal response. Three factors explain this: (1) The training dataset contains only 365 examples across 51 command types, giving the model limited memorization material. (2) Many test commands are complex chained operations (`apt update && apt install...`, `chmod +x ... ; nohup ...`) where partial response correctness is penalized harshly. (3) Comparable domain-specific fine-tuning studies with small datasets report BLEU scores of 0.10–0.25. The 90% consistency rate demonstrates that despite low BLEU, the model produces stable and deterministic outputs — a more operationally relevant property for honeypot deployment.

---

**Q10: What does the 76.7% hallucination rate mean for deployment?**

The hallucination rate is measured as the percentage of samples with ROUGE-L F1 < 0.30 compared to ground truth. This threshold-based metric conflates two distinct phenomena: (1) true hallucination (fabricated, non-Linux outputs) and (2) paraphrasing (valid but textually different Linux responses). Manual inspection reveals that many "hallucinated" samples produce structurally plausible terminal outputs that simply differ from the specific ground-truth template. For deployment purposes, what matters is whether the output deceives an attacker — not whether it matches a template exactly. The high consistency rate (90%) suggests the model maintains coherent behavior, which is more important for sustained deception than exact template matching.

---

**Q11: How did you address overfitting in the LLM fine-tuning dataset?**

We performed information entropy analysis on the combined dataset before training. The initial 8,006-entry dataset had a Shannon entropy of only 1.14 bits (19.8% of maximum), with 85.8% of entries being `su <user>` commands producing identical `Password: \nsu: Authentication failure` output. This would cause the model to overfit to a single response pattern. We applied three corrections: (1) capped `su` entries to 50 diverse usernames, (2) removed entries with empty outputs, (3) shuffled the dataset. The resulting 365-entry dataset achieved 3.89 bits entropy (68.6% efficiency) across 51 base commands and 151 unique outputs — a 3.4× entropy improvement.

---

## 10. Final Positioning Statement

This project successfully implements a scoped Honeypot Behavioral Intelligence Model with an experimental LLM-enhanced deception component. While not designed as a full enterprise IDS, it provides a scientifically structured foundation for behavioral threat intelligence within controlled honeypot environments.

**ML Behavioral Classification:** The system demonstrates that structured behavioral feature extraction from heterogeneous honeypot sources — combined with **comprehensive leakage-aware separation** of heuristic indicators, direct label-determining features, and proxy leakage features from ML training — can produce classifiers that identify automation patterns (F1=0.99), tunneling behavior (F1=0.97), and multi-label attack categories (macro-F1=0.95) with high precision and recall. Feature importance analysis validates that models learn from genuine behavioral signals (entropy, command structure, payload characteristics) rather than leaked deterministic shortcuts. Critically, the two-round leakage fix ensures that these scores reflect genuine behavioral learning rather than trivial rule re-derivation. **5-fold stratified cross-validation** further confirms the reliability of these metrics, with standard deviations consistently below ±0.01 across all tasks and models, demonstrating that the reported performance is reproducible and not an artifact of a single favorable data split.

**LLM-Enhanced Deception:** The fine-tuned Phi-3-mini model (QLoRA, 365-sample dataset) demonstrates **90% output consistency** — a critical requirement for honeypot deployment where response instability would alert attackers. The BLEU (0.16) and ROUGE-L (0.23) scores reflect the challenge of exact-match evaluation on a small, diverse training corpus; these are within the expected range for domain-specific fine-tuning with limited data. The 76.7% hallucination rate (ROUGE-L < 0.30 threshold) indicates that the model frequently produces outputs differing from ground-truth templates, though manual inspection suggests many are structurally valid Linux responses. The information entropy analysis and dataset rebalancing methodology (entropy improvement from 1.14 to 3.89 bits) represent a transferable contribution to LLM fine-tuning quality assurance. Future work should focus on expanding the training corpus, implementing expert-reviewed hallucination classification, and deploying the model in a live Cowrie environment for real-world deception measurement.
