 """
  infer.py - Bridge between the SOC Dashboard and the ML Models

  This script is called by the ML Predictor tab in the dashboard.
  It runs machine learning predictions on attacker sessions in two modes:

    1. Session mode (--session <id>)
       Reads raw events from the database, computes 24 features from them
       (command counts, timing delays, entropy, ports, etc.), then runs
       all three models and returns features + predictions as JSON.

    2. Features mode (--features <json>)
       Skips the database. Takes feature values directly and runs the
       three models on them. Used when the user edits features manually
       in the dashboard.

  Models:
    - Bot Detection    (RandomForest) : is this session a bot or human?
    - Tunnel Detection (XGBoost)      : is data tunneling present?
    - Multi-Label      (XGBoost)      : which of 9 attack types apply?

  Note: Features are computed identically to how they were computed
  during training, ensuring consistent and accurate predictions.
  """
